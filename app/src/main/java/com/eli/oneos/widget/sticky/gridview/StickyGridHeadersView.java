        ...
        if (mHeaderChildBeingPressed) {
            ...
            if (tempHeader != null) {
                ...
                tempHeader.postDelayed(new Runnable() {
                    public void run() {
                        invalidate(0, headerHolder.getTop(), getWidth(), headerHolder.getTop()
                                + headerHolder.getHeight());
                    }
                ...
                if (!wasHeaderChildBeingPressed) {
                    if (header != null) {
                        ...
                        if (mTouchMode == TOUCH_MODE_DOWN || mTouchMode == TOUCH_MODE_TAP) {
                            ...
                            if (!mDataChanged) {
                            /*
                             * Got here so must be a tap. The long press would have triggered on the
							 * callback handler.
							 */
...
//        mStickiedHeader.measure(MeasureSpec.makeMeasureSpec(0, 0), MeasureSpec.makeMeasureSpec(0, 0));
    ...
    private MotionEvent transformEvent(MotionEvent e, int headerPosition) {
        ...
        MotionEvent n = MotionEvent.obtain(downTime, eventTime, action, pointerCount, pointerIds,
...
//            mStickiedHeader.measure(MeasureSpec.makeMeasureSpec(0, 0), MeasureSpec.makeMeasureSpec(0, 0));
    ...
    public int getCountFromHeader(int id) {
        if (null != mAdapter) {
            return mAdapter.getShowCountFromHeader(id);
        }
