...
package com.eli.oneos.widget.sticky.gridview;
...
public class StickyGridHeadersBaseAdapterWrapper extends BaseAdapter {
    ...
    public int getCount() {
        ...
        for (int i = 0; i < numHeaders; i++) {
            ...
            // TODO..
            mCount += mDelegate.getShowCountForHeader(i) + unFilledSpacesInHeaderGroup(i)
                    + mNumColumns;
        }
        mCounted = true;
        return mCount;
    ...
    public int getShowCountFromHeader(int id) {
        if (null != mDelegate) {
            return mDelegate.getShowCountForHeader(id);
        }
    ...
    /**
     * swith girdview header item state
     *
     * @param position
     */
    public void toggleHeader(long position) {
        if (null != mDelegate) {
            Log.e("StickyGridHeadersBaseAdapterWrapper", "===============On Header Click: " + position);
            mDelegate.toggleHeader(position);
        }
    }
    ...
    private int unFilledSpacesInHeaderGroup(int header) {
        // TODO..
        int remainder = mDelegate.getShowCountForHeader(header) % mNumColumns;
    ...
    protected Position translatePosition(int position) {
        ...
        for (i = 0; i < numHeaders; i++) {
            ...
            // TODO..
            int sectionCount = mDelegate.getShowCountForHeader(i);
            if (place < sectionCount) {
                // TODO.. 修正Header Item位置，跳过隐藏的Item
                if (i > 0) {
                    for (int j = 0; j < i; j++) {
                        int showCount = mDelegate.getShowCountForHeader(j);
                        if (showCount == 0) {
                            adapterPosition += mDelegate.getCountForHeader(j);
                        }
                    }
                }
    ...
    protected void updateCount() {
        ...
        for (int i = 0; i < numHeaders; i++) {
            // TODO..
            mCount += mDelegate.getShowCountForHeader(i) + mNumColumns;
        }
        mCounted = true;
    }
