...
public class StickyGridHeadersSimpleAdapterWrapper extends BaseAdapter implements
	...
	private static final String TAG = StickyGridHeadersSimpleAdapterWrapper.class.getSimpleName();
	...
	public int getCountForHeader(int position) {
		if (position >= mHeaders.length) {
			Log.e(TAG, "Array out of bounds");
			return 0;
		}
		// String pos = String.valueOf(position);
		// if (mFoldList.contains(pos)) {
		// return 0;
		// }
	...
	@Override
	public int getShowCountForHeader(int position) {
		if (position >= mHeaders.length) {
			Log.e(TAG, "Array out of bounds");
			return 0;
		}
	...
	public void toggleHeader(long position) {
		if (null != mHeaders && position < getNumHeaders()) {
			String pos = String.valueOf(position);
			int index = mFoldList.indexOf(pos);
			if (index == -1) {
				mFoldList.add(pos);
			} else {
				mFoldList.remove(index);
			}
		}
	}
